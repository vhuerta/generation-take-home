...
var GET_STORES = exports.GET_STORES = function GET_STORES(req, res, next) {
  _redis2.default.getAsync("STORES" // Try to get the stores from redis
    ...
    return res ? JSON.parse(res) : _stores2.default.find({ geolocation: { $ne: null, $exists: true } }).lean();
  } // If not found get from database
  ).then(function (stores) {
    return _redis2.default.setAsync("STORES", JSON.stringify(stores)), res.out({
      code: 200,
      data: { stores: stores }
    });
  }).catch(function (err) {
